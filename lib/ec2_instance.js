  ...
  */
  ...
  S['getS3File'] = function(options, callback){
    var a = Belt.argulint(arguments)
      , self = this;
    a.o = _.defaults(a.o, {
      'path': self.settings.__dirname
    // s3path - path on s3 bucket to file
    // bucket - s3 bucket
    // name - optional filename, defaults to name on s3
    });

    if (!a.o.s3path || !a.o.bucket) return a.cb(new Error('bucket and s3path are required'));

    if (!a.o.name) a.o.name = a.o.path.split('/').pop();

    return self.s3.getFile(a.o.bucket, a.o.s3path, function(err, body){
      if (err) return a.cb(err);

      var p = Path.join(a.o.path, '/', a.o.name);
